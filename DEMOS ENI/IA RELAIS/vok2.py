...
# PRESENTABLE AVEC KYUUBI EXTRACTLITE()
# MAIS
# 1EM : 01 ROUGE VEHICULE ENTRE LAISSER POUR COMPTE
# 2EM : ON A LAISSE LA CAISSE 4*4 DANS LA VIDEO POUR
# MONTRE L IMPORTANCE DU RESPECT DE LA CODE DE LA ROUTE
...
def video3():
    ...
    video = '/home/trofel/Bureau/DEMOS ENI/DATA/ok2.mp4'

    vehicule1 = 0
    vehicule2 = 0
    seuil = 70  # ou 45 minim
    seuil2 = 147
    ...
    while True:
        ...
        cadre1 = frame[ymin:ymax, xmin1 - x1_offset:xmax1]
        cadre2 = frame[ymin:ymax, xmin2:xmax2 + x2_offset]

        if calcul_mean(mask1[0:ymax - ymin, 0:xmax1 - xmin1]) > seuil2:
            if old_1 == 0:
                ku.Create(file)
                vehicule1 += 1
                ku.API(file, vehicule1, cadre1, 'E', 'ok2')
                old_1 = 1
                ku.Delete(file)
        else:
            old_1 = 0
        ...
        if calcul_mean(mask2[0:ymax - ymin, 0:xmax2 - xmin2]) > seuil2:
            if old_2 == 0:
                ku.Create(file)
                vehicule2 += 1
                ku.API(file, vehicule2, cadre2, 'S', 'ok2')
                old_2 = 1
                ku.Delete(file)
        else:
            old_2 = 0
        ...
        cv2.putText(frame, "OUT:{:02d}".format(vehicule2), (xmin2, ymin - 10), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2,
                    (255, 255, 255), 2)
        cv2.line(frame, (xmin1 - 30, ymin), (xmax1, ymin), (0, 0, 255) if old_1 else (255, 0, 0), 3)
        cv2.line(frame, (xmin2 - 10, ymin), (xmax2, ymin), (0, 0, 255) if old_2 else (255, 0, 0), 3)
